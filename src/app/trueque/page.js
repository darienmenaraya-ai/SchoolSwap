...
export default function Trueque() {
  ...
  function ProductoCard({ producto, label }) {
    return (
      <div className="rounded-xl p-4 text-center border" style={{ backgroundColor: '#f5f7ff', borderColor: '#e2e6ff' }}>
        <p className="text-xs font-bold mb-2" style={{ color: '#6b7280' }}>{label}</p>
        <div className="w-20 h-20 rounded-lg mx-auto mb-2 overflow-hidden flex items-center justify-center" style={{ backgroundColor: '#e8eaff' }}>
          {producto?.imagen ? (
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: '#f5f7ff' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-12">
        ...
        <p className="mb-8" style={{ color: '#6b7280' }}>Intercambiá productos sin usar dinero</p>
          ...
          <div className="space-y-12">
            <div>
              ...
              {propuestasRecibidas.length === 0 ? (
                <div className="bg-white border rounded-xl p-8 text-center shadow-sm" style={{ borderColor: '#e2e6ff' }}>
                  <p style={{ color: '#6b7280' }}>No tenés propuestas de trueque recibidas</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {propuestasRecibidas.map((t) => (
                    <div key={t.id_trueque} className="bg-white border rounded-xl p-6 shadow-sm" style={{ borderColor: '#e2e6ff' }}>
                      ...
                      <div className="grid grid-cols-3 gap-4 items-center">
                        ...
                        <div className="text-center">
                          <RefreshCw size={32} className="mx-auto" style={{ color: '#3b4fd8' }} />
                          <p className="text-xs mt-1" style={{ color: '#6b7280' }}>a cambio de</p>
                        </div>
                      ...
                      {t.estado === 'pendiente' && (
                        <div className="flex gap-3 mt-4">
                          <button onClick={() => responderTrueque(t.id_trueque, 'aceptado')}
                            ...
                            style={{ backgroundColor: '#1a1f6e', color: 'white' }}>Aceptar</button>
                          <button onClick={() => responderTrueque(t.id_trueque, 'rechazado')}
                            ...
                            style={{ borderColor: '#fca5a5', color: '#dc2626', backgroundColor: 'white' }}>Rechazar</button>
                        </div>
                      )}
                    </div>
                  ))}
            ...
            <div>
              ...
              {propuestasEnviadas.length === 0 ? (
                <div className="bg-white border rounded-xl p-8 text-center shadow-sm" style={{ borderColor: '#e2e6ff' }}>
                  <p style={{ color: '#6b7280' }}>No has enviado propuestas de trueque aún</p>
