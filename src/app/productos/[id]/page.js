            ...
            <div className="p-8 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {producto.stock > 0 ? (
                    <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ backgroundColor: '#f0fdf4', color: '#166534' }}>En stock</span>
                  ) : (
                    <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ backgroundColor: '#fef2f2', color: '#dc2626' }}>Agotado</span>
                  )}
                </div>
                ...
                {usuario?.id !== producto.id_usuario ? (
                  <div className="space-y-3">
                    <button onClick={agregarAlCarrito} disabled={agregando || producto.stock === 0}
                      ...
                      style={{ backgroundColor: producto.stock === 0 ? '#e5e7eb' : '#1a1f6e', color: producto.stock === 0 ? '#9ca3af' : 'white', cursor: (agregando || producto.stock === 0) ? 'not-allowed' : 'pointer', boxShadow: producto.stock > 0 ? '0 4px 15px rgba(26,31,110,0.3)' : 'none' }}>
